...
class LoadUsecase:
    ...
    def fit_dress_morph(
        ...
        for i, (bone_name, bone_setting) in enumerate(DRESS_STANDARD_BONE_NAMES.items()):
            ...
            # if bone_setting.category in dress_category_global_scales:
            #     dress.morphs[DRESS_BONE_FITTING_NAME].offsets.append(
            #         BoneMorphOffset(
            #             dress_bone.index,
            #             scale=dress_category_global_scales[bone_setting.category],
            #         )
            #     )
            #     dress_global_scales[dress_bone.index] = (
            #         dress_global_scales.get(dress_bone.index, MVector3D()) + dress_category_global_scales[bone_setting.category]
            #     )
    ...
    #                 child_bone_indexes = [
    #                     child_bone_index
    #                     for child_bone_index in dress_other_bone.child_bone_indexes
    #                     if dress.bones[child_bone_index].is_standard
    #                 ]
    #             if not child_bone_indexes:
    #                 child_bone_indexes = dress_parent_bone.child_bone_indexes
    #             if not child_bone_indexes:
    #                 child_bone_indexes = dress_other_bone.child_bone_indexes
    ...
    #                 dress.bones[child_bone_index].position
    #                 if 0 <= child_bone_index
    #                 else dress_other_bone.position + dress_other_bone.tail_relative_position
    #             )

    #             target_bone_names = [dress_parent_bone.name, dress_other_bone.name]
    #             if 0 <= child_bone_index:
    #                 target_bone_names.append(dress.bones[child_bone_index].name)
    ...
    #                 dress_matrixes[0, dress.bones[child_bone_index].name].position
    #                 if 0 <= child_bone_index
    #                 else dress_matrixes[0, dress_other_bone.name].global_matrix * dress_other_bone.tail_relative_position
    #             )
    ...
    #     prime_local_positions: dict[str, np.ndarray] = {}
    #     for n, bone_name in enumerate(LOCAL_SCALE_WEIGHT_BONE_NAMES):
    #         bone_setting = DRESS_STANDARD_BONE_NAMES[bone_name]

    #         if bone_name in prime.bones:
    #             prime_bone = prime.bones[bone_name]
    #             prime_vertices = set(prime.vertices_by_bones.get(prime.bones[bone_name].index, []))
    #             if prime_vertices:
    #                 prime_deformed_local_positions = self.get_deformed_local_positions(
    #                     prime, prime_bone, bone_setting, prime_vertices, prime_matrixes
    #                 )

    #                 if bone_setting.category not in prime_local_positions:
    #                     prime_local_positions[bone_setting.category] = prime_deformed_local_positions
    #                 else:
    #                     prime_local_positions[bone_setting.category] = np.vstack(
    #                         (prime_local_positions[bone_setting.category], prime_deformed_local_positions)
    #                     )
    ...
    #     for category in dress_local_positions.keys():
    #         if category not in prime_local_positions:
    #             continue

    #         # フィルタした値
    #         prime_filtered_local_positions = filter_values(prime_local_positions[category])
    #         dress_filtered_local_positions = filter_values(dress_local_positions[category])

    #         prime_local_distances = np.max(prime_filtered_local_positions, axis=0) - np.min(prime_filtered_local_positions, axis=0)
    ...
    #         logger.debug(
    #             f"ローカルスケール [{category}][{dress_local_scales[category]}][prime={prime_local_distances}]"
