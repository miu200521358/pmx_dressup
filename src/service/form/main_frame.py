...
class MainFrame(NotebookFrame):
        ...
        if self.notebook.GetSelection() == self.config_panel.tab_idx:
            ...
            if not self.load_worker.started:
                ...
                if (
                    ...
                    self.file_panel.motion_ctrl.path
                    and not self.file_panel.motion_ctrl.data
                ):
                    self.file_panel.exec_btn_ctrl.Enable(False)
    ...
    def on_result(
        ...
        if not (
            ...
            and self.file_panel.motion_ctrl.data
        ):
            logger.warning(
        ...
        all_material_names = (
            ...
            + [
                __("衣装") + ":" + material_name
                for material_name in dress.materials.names
            ]
        )

        # 材質の選択肢を入れ替える
        self.config_panel.model_material_ctrl.initialize(
            model.materials.names, all_material_names
        )
        self.config_panel.dress_material_ctrl.initialize(
            dress.materials.names, all_material_names
        )
        # モーフの選択肢を入れ替える
        self.config_panel.model_morph_ctrl.initialize(
            [m.name for m in model.morphs if not m.is_system]
        )
        self.config_panel.dress_morph_ctrl.initialize(
            [m.name for m in dress.morphs if not m.is_system]
        )
        # ボーン調整の選択肢を入れ替える
        self.config_panel.dress_bone_ctrl.initialize(
            individual_morph_names, individual_target_bone_indexes
        )

        try:
            logger.info("人物モデル描画準備")
            self.config_panel.canvas.append_model_set(
                self.file_panel.model_ctrl.data, self.model_motion, bone_alpha=0.5
            )
    ...
    def on_motion_result(
        self, result: bool, data: Optional[VmdMotion], elapsed_time: str
    ) -> None:
        self.file_panel.console_ctrl.write(f"\n----------------\n{elapsed_time}")
    ...
    def on_save_result(
        self, result: bool, data: Optional[Any], elapsed_time: str
    ) -> None:
        self.file_panel.Enable(True)
        self.on_sound()

    def set_model_motion_morphs(
        self,
    ...
    def fit_model_motion(
        self, bone_alpha: float = 1.0, is_bone_deform: bool = True
    ) -> None:
    ...
    def change_bone(self, selected_bone_indexes: list[int]) -> None:
        self.config_panel.canvas.animations[
            1
        ].selected_bone_indexes = selected_bone_indexes
        self.config_panel.canvas.Refresh()
